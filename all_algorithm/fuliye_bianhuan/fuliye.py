...
# 计算离散傅里叶逆变换：xn = R1 / N * ∑ xk * e**(j*2pi*k*j/N)
def IDFT_slow(xk):
    N = xk.shape[0]
    j = np.arange(N)
    k = j.reshape((N, 1))
...
#FFT进行
def FFT(xn):
    print('xn是什么'+str(xn))   #补全A，B后的高项系数
    N = xn.shape[0]

    N_min = min(N, 32)

    print('N_min'+str(N_min))   #32 ,强行补全不需要计算，下面就要拿选择的W进入了。
    AX = DFT_slow(xn=xn.reshape((N_min, -1)))    #不太懂
...
def IFFT(xk):
    ...
    while AX.shape[0] < N:
        AX_even = AX[:, :int(AX.shape[1] / 2)]    # 偶数项多项式
        AX_odd = AX[:, int(AX.shape[1] / 2):]      # 偶数项多项式
        factor = np.exp(2j * np.pi * np.arange(AX.shape[0]) / AX.shape[0] / 2)
        factor = factor.reshape((factor.shape[0], 1))
        factor_AX_odd = factor * AX_odd
        AX = np.vstack([AX_even + factor_AX_odd, AX_even - factor_AX_odd])
...
def FFT_multiply(N, x1_vec, x2_vec):
    ...
    print(  "关于A的点值表达式，A是多少"+str(A.shape))
    B = FFT(xn=np.array(x2_vec + [0] * (N - len(x2_vec))))
    print()
    print(  "关于B的点值表达式，B是多少"+str(B.shape))

    C = A * B
    print()
    print(  "关于C的点值表达式，C是多少"+str(C.shape))
    D = IFFT(xk=C)      #逆IFFT,给出点值表达式就可以反过来求系数。
    print(  "关于D的点值表达式，D是多少"+str(D))
    return calc_all_number(D)
...
if __name__ == '__main__':
    ...
    x1_vec = list(map(int, list(reversed(str(x1)))))    #反转数字，成list
    print("x1_vec"+str(x1_vec))
    x2_vec = list(map(int, list(reversed(str(x2)))))     #反转数字，成list


    # R1、加倍次数界：由分治法的思想，将两个多项式的次数界补全为2的幂次,补全
    extra_len = 1
